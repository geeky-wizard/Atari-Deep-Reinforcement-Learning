...
def train(env,agent):
    ...
    for episode in range(no_episodes):
        ...
        # print(episode+1, "\tTotalReward = ", score, "\tSteps: ", steps, "\tMoving Avg: {:.2f}".format(avg_score),"\tTime: %d" % (timer/60), "\b:{:02.0f}".format((timer % 60)))
        print(episode+1, "\tTotalReward = ", score, "\tSteps: ", steps, "\tMoving Avg: {:.2f}".format(avg_score),"\t Episilon = %f "%(agent.epsilon),"\tTime: %d" % (timer/60), "\b:{:.0f}".format((timer % 60)))
        agent.save()
